...
Now, we will take a  look at how the data is traversing \texttt{scipy} after being called in line 571 of listing \ref{lst:scikitTOscipy}.
\bigskip
...
\begin{itemize}
	...
	\item \texttt{svds} then analyses the shape of the matrix in order to determine the most efficient sequence of multiplication (either $A^TA$ or $AA^T$) before passing it to the next function \texttt{eigsh}.
	\item \texttt{eigsh} then verifies the content of the matrix and chooses the adequate ARPACK driver:
	\begin{itemize}
		\item If the matrix is identified an Hermitian (complex) matrix, then the complex driver is used. As we are only focusing on real (symmetric) matrices, this step wont be considered any further.
...
\texttt{SAUPD} can then be finetuned with various parameters such as \texttt{WHICH} to computed the smallest or largest eigenvalues first, \texttt{NEV} to set the numbers of eigenvalues to be computed, \texttt{TOL} to set the tolerance of the precision for floating point numbers.
\bigskip
...
As a last approach to establishing a \acrlong{svd} of a matrix, we will take a look at opportunities that are made possible by random sampling. 
The primary driver to random methods are the increasingly vast measurements available across many different disciplines.
...
The number of column in $P$, $r$ represent the number of \acrlongpl{pc} we want to compute.
...
Now it is time to compute the low-rank $QR$ decomposition of $Z$.
$Q \in \mathbb{R}^{n \times r}$, $R \in \mathbb{R}^{r \times r}$. 
This will result in $Z$ being an orthonormal basis for $X$:
...
Finally, thanks to aforementioned properties, it is now possible to recontsruct the high-dimensional left singular vectors $U$ using $U_Y$ and $Q$:
...
This being said, we are unlikely to guess it right.
...
The idea is to preprocess a data matrix $X$ and to take it to a parametrised power $q$.
This results in:
...
Using power iterations, we lenghten the distance between the significant ranks and the redundant information.
...
The gain in efficiency is not solely owed to the nature of computing based on random lower dimesional matrices but also additionally allows efficient processing in multi-processor architectures \cite{halko2011finding}.
