...
The parameter \texttt{random\_state} will make sure that the random vectors are only pseudorandom and provide a deterministic result.
...
\begin{itemize}
	\item Starting in \texttt{scikit}, we arrive in the second node \texttt{svds}, which represents a function in the \texttt{scipy} module \texttt{scipy.sparse.linalg.svds}.
\end{itemize}
