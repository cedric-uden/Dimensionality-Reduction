...
% For support in macOS, `inkscape` might has to be linked into `/usr/local/bin` as the path does not seem to take into consideration homebrew install.
% run `sudo ln /opt/homebrew/bin/inkscape /usr/local/bin/`
...
\newcommand{\tikzscale}{1.25}

% override warning "Difference (2) between bookmark levels is greater (hyperref) than one, level fixed"
% caused by having skipped a section hierarchy. i.e. from "3" to "3.0.1"
\usepackage{bookmark}
